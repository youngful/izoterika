		...
		<div className='flex 2xs:gap-[200px] 2xs:justify-center 2lg:justify-between  2lg:gap-[40px] flex-wrap'>
			{list.map((item, index) => (
				<div
					...
					} max-w-[570px] w-full border-[1px] border-[#247CA8] relative p-[80px_56px] rounded-[32px]`}
					key={index}
				>
					...
					<h3 className='uppercase text-center gradient_text mb-[50px]'>{item.title}</h3>

					<ul>
						{item.list.map((listItem, listIndex) => (
							<div key={listIndex} className={`flex gap-4 items-center mb-4 ${!listItem.Include && 'line-through text-[#3c93b5] opacity-50 '}`}>
								<span className={`${!listItem.Include && 'w-[8px] h-[8px] ml-[2px]'} w-[12px] h-[12px] rounded-full bg-[#247CA8]`}></span>
								<li className='font-light max-w-[370px] w-full break-words'>{listItem.title}</li>
							</div>
						))}
					</ul>

					<div className='flex items-center font-semibold justify-center mt-[50px] 2xs:gap-[24px] xs:gap-[50px]'>
					...
					<a
						className='primary_btn max-w-[calc(100%-80px)] absolute left-[50%] translate-x-[-50%] translate-y-[-50%] bottom-[-100px]'
						href={item.link}
					>
						Отримати курс
					</a>
