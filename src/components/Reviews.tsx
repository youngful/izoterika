...
export default function Reviews({ reviews }: ReviewsProps) {
	return (
		<div className='grid 2xs:mx-auto 2xs:max-w-[630px] lg:max-w-[1028px] lg:grid-cols-[500px_500px] 2xs:gap-[14px] lg:gap-[28px]'>
			{reviews.map((review, index) => (
				<div key={index} className='p-[30px_36px] bg-white rounded-[20px]'>
					<div className='flex flex-col items-end gap-2 mb-[28px]'>
						<StarRating rating={review.rating} />
						<p className='sm:text-[30px] sm:leading-[36px]'>{review.name}</p>
					</div>
